...
{
  t.tx_buffer= &(R_REGISTER | ( REGISTER_MASK & reg )) ;  
...
{
  t.tx_buffer= (uint8_t *)&(R_REGISTER | ( REGISTER_MASK & reg )) ;  
...
uint8_t RF24_write_register_multi_len(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  t.tx_buffer= &(W_REGISTER | ( REGISTER_MASK & reg )) ;               
  ret=spi_device_transmit(spi, &t);  //Transmit!
...
uint8_t RF24_write_register_single_len(uint8_t reg, uint8_t const uint8_t *value)
{
  ESP_LOGI(TAG, "RF24_write_register_single_len(%02x,%02x)\r\n", reg,value);


  t.tx_buffer= &(W_REGISTER | ( REGISTER_MASK & reg )) ;               //Data
  ret=spi_device_transmit(spi, &t);  //Transmit!
...
void RF24_begin(void)
{
  ...
  RF24_write_register_single_len(STATUS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void RF24_startListening(void)
{
  ...
  if (pipe0_reading_address)
    RF24_write_register_multi_len(RX_ADDR_P0, reinterpret_cast<const uint8_t*>(&pipe0_reading_address), 5);
...
bool available(uint8_t* pipe_num)
{
  ...
  if (result)
  {
    ...
    RF24_write_register_single_len(STATUS,_BV(RX_DR) );

    // Handle ack payload receipt
    if ( status & _BV(TX_DS) )
    {
      RF24_write_register_single_len(STATUS,_BV(TX_DS));
    }
  }
...
void RF24_whatHappened(bool tx_ok,bool tx_fail,bool rx_ready)
{
  ...
  uint8_t status = RF24_write_register_single_len(STATUS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void RF24_openWritingPipe(uint64_t value)
{
  ...
  RF24_write_register_multi_len(RX_ADDR_P0, reinterpret_cast<uint8_t*>(&value), 5);
  RF24_write_register_multi_len(TX_ADDR, reinterpret_cast<uint8_t*>(&value), 5);

  const uint8_t max_payload_size = 32;
  RF24_write_register_multi_len(RX_PW_P0,min(payload_size,max_payload_size));
}
...
//     if ( child < 2 )
//       RF24_write_register_multi_len(pgm_read_byte(&child_pipe[child]), reinterpret_cast<const uint8_t*>(&address), 5);
//     else
//       RF24_write_register_multi_len(pgm_read_byte(&child_pipe[child]), reinterpret_cast<const uint8_t*>(&address), 1);
...
void RF24_writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  t.tx_buffer= &(W_ACK_PAYLOAD | ( pipe & 0x7 )) ;               
  ret=spi_device_transmit(spi, &t);  //Transmit!
